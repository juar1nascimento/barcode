...
def renderizar_card_saida(lista_urs, lista_ubs):
    with st.container(border=True):
        ...
        unidade_escolhida = (
            urs_saida if urs_saida != "Selecione uma URS..."
            else ubs_saida if ubs_saida != "Selecione uma UBS..."
            else ""
        )
        if unidade_escolhida:
            st.session_state.saved_setor = unidade_escolhida
...
def renderizar_sistema_saida():
    ...
    if st.button("🚨 Registrar Saída de Equipamento", type="primary", use_container_width=True):
        if not setor_atual or setor_atual == "Unidade não selecionada":
            st.warning("Selecione uma URS ou UBS antes de registrar a saída.")
        elif codigo_saida.strip():
