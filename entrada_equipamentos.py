...
def renderizar_card_entrada(lista_urs, lista_ubs):
    with st.container(border=True):
        ...
        unidade_escolhida = (
            urs_entrada if urs_entrada != "Selecione uma URS..."
            else ubs_entrada if ubs_entrada != "Selecione uma UBS..."
            else ""
        )
...
def renderizar_sistema_entrada():
    ...
    if st.button("✅ Confirmar Entrada de Equipamento", type="primary", use_container_width=True):
        ...
        if not setor_atual or setor_atual == "Unidade não selecionada":
            st.warning("Selecione uma URS ou UBS antes de confirmar a entrada.")
        elif valor_final:
