...
def renderizar_login() -> bool:
    ...
    if "autenticado" not in st.session_state:
        st.session_state.autenticado = False[cite: 18]

    if st.session_state.autenticado:
        return True[cite: 18]

    # Codificação segura em Base64 do SVG
    b64_logo = base64.b64encode(LOGO_SERRA_SVG.encode("utf-8")).decode("utf-8")[cite: 18]
    logo_html = f'<div style="text-align: center; margin-bottom: 25px;"><img src="data:image/svg+xml;base64,{b64_logo}" width="280" /></div>'[cite: 18]
    ...
    _, col_center, _ = st.columns([1, 1.8, 1])[cite: 18]

    with col_center:
        # Renderização segura da Logo
        st.markdown(logo_html, unsafe_allow_html=True)[cite: 18]

        # Form de Login
        with st.form(key="glpi_login_form", clear_on_submit=False):[cite: 18]
            st.markdown('<div class="login-title">Faça login na sua conta</div>', unsafe_allow_html=True)[cite: 18]

            st.write("**Usuário**")[cite: 18]
            usuario = st.text_input("Usuário", value="", label_visibility="collapsed", key="login_user")[cite: 18]

            st.write("**Senha**")[cite: 18]
            # Link ajustado com URL de destino e abertura em nova aba (target="_blank")
            st.markdown(f'<a href="{URL_ESQUECEU_SENHA}" target="_blank" class="forgot-link">Esqueceu sua senha?</a>', unsafe_allow_html=True)[cite: 18]
            senha = st.text_input("Senha", value="", type="password", label_visibility="collapsed", key="login_pass")[cite: 18]

            st.write("**Origem de login**")[cite: 18]
            origem = st.selectbox("Origem de login", ["SERRA.LOCAL", "BANCO DE DADOS INTERNO"], label_visibility="collapsed", key="login_domain")[cite: 18]

            submit = st.form_submit_button("Entrar", use_container_width=True)[cite: 18]

            if submit:[cite: 18]
                if usuario.strip() != "" and senha.strip() != "":[cite: 18]
                    st.session_state.autenticado = True[cite: 18]
                    st.session_state.erro_login = False[cite: 18]
                    st.rerun()[cite: 18]
                else:
                    st.session_state.erro_login = True[cite: 16, 18]

        # Alerta de erro
        if st.session_state.get("erro_login", False):[cite: 18]
            st.markdown("""
                <div class="error-box">
