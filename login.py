...
LOGIN_BACKGROUND_FILE = Path(__file__).resolve().parent / "assets" / "login_background.svg"
...
def processar_acao_via_url():
    ...
    if "acao" in params and "usuario" in params:
        ...
        if acao == "aprovar":
            ...
            <p>Seu cadastro/solicitação de acesso para o e-mail <b>{html.escape(user_email)}</b> foi <b>ACEITO</b> pelo administrador.</p>
            ...
            corpo = f"""
            <h3>Prefeitura Municipal da Serra</h3>
            <p>Sua solicitação de cadastro para o e-mail <b>{html.escape(user_email)}</b> foi <b>RECUSADA</b> pelo administrador.</p>
            """
...
def _carregar_fundo_login_b64() -> str:
    """Carrega o fundo local do repositório, evitando dependência de URL externa."""
    try:
        return base64.b64encode(LOGIN_BACKGROUND_FILE.read_bytes()).decode("ascii")
...
def renderizar_login() -> bool:
    ...
    b64_background = _carregar_fundo_login_b64()
    background_html = ""
    if b64_background:
        background_html = f'''<div class="login-background-layer" aria-hidden="true"><img src="data:image/svg+xml;base64,{b64_background}" alt="" /></div>'''
    ...
    st.markdown("""
        <style>
            ...
            .stApp { position: relative !important; isolation: isolate !important; }
            ...
            [data-testid="stAppViewContainer"] > .main { position: relative !important; z-index: 1 !important; }
            ...
            .login-logo-wrap { width: 100%; display: flex; justify-content: center; align-items: center; margin: 0 auto 2.15rem auto; line-height: 0; }
            .login-logo { width: 230px; height: auto; max-width: 78vw; display: block; image-rendering: auto; }
            ...
            .login-title { text-align: center; font-size: 20px; line-height: 1.25; font-weight: 600; color: #24292e; margin: 0 0 25px 0; }
            button[kind="tertiary"] {
                display: flex !important; justify-content: flex-end !important; width: 100% !important; margin-left: auto !important;
                font-size: 12px !important; color: #24292e !important; text-decoration: underline !important;
                margin-top: -10px !important; margin-bottom: 15px !important; padding: 0 !important; height: auto !important;
            ...
            .error-box { width: 100%; max-width: 810px; box-sizing: border-box; margin: 15px auto 0 auto; background-color: #fdf2f2; border: 1px solid #f8b4b4; border-left: 4px solid #e02424; color: #9b1c1c; padding: 12px 16px; border-radius: 4px; font-size: 13px; }
                    ...
                    if not validar_email(novo_user_clean):
                        ...
                        if not senha_ok:
                            ...
                            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
                                ...
                                <ul><li><b>E-mail/Usuário Solicitado:</b> {html.escape(novo_user_clean)}</li></ul>
                                <p>Clique em uma das opções abaixo para responder à solicitação diretamente:</p>
                                <div style="margin-top: 25px;">
                                    <a href="{html.escape(link_aprovar, quote=True)}" style="background-color: #28a745; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; margin-right: 15px; display: inline-block;">Autorizar Cadastro</a>
                                    <a href="{html.escape(link_recusar, quote=True)}" style="background-color: #dc3545; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block;">Recusar Cadastro</a>
                                </div>
                            </div>
                            """
