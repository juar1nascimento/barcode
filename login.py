    ...
    with center:
        ...
        if st.session_state.tela_atual == "redefinicao_solicitar":
            with st.form("form_solicitar_email", clear_on_submit=False):
                st.markdown('<div class="login-title">Redefinição de senha</div>', unsafe_allow_html=True); st.write("**Informe seu e-mail de acesso**")
            ...
            with st.form("form_criar_usuario",clear_on_submit=False):
                st.markdown('<div class="login-title">Redefinição de senha</div>',unsafe_allow_html=True); st.write("**Login de Usuário (Obrigatório ser E-mail)**")
