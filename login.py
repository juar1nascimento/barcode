...
# Logo da Prefeitura da Serra em SVG Vetorial Nativo (Alta Definição)
LOGO_SERRA_SVG = """
...
def renderizar_login() -> bool:
    ...
    # Estilização CSS global e direcionada ao Formulário
    st.markdown("""
        <style>
            ...
            /* Oculta cabeçalho/rodapé nativos */
            header, footer, #MainMenu {
                visibility: hidden;
            }

            /* Estiliza o Form do Streamlit como Card do GLPI */
    ...
    # Centralização usando layout nativo
    _, col_center, _ = st.columns([1, 1.8, 1])

    with col_center:
        # Logo Superior
        st.markdown(LOGO_SERRA_SVG, unsafe_allow_html=True)
        ...
        with st.form(key="glpi_login_form", clear_on_submit=False):
            ...
            st.write("**Senha**")
            st.markdown('<a href="#" class="forgot-link">Esqueceu sua senha?</a>', unsafe_allow_html=True)
        ...
        st.markdown("""
            <div class="login-footer">
                GLPI Copyright (C) 2015-2026 Teclib' and contributors
            </div>
        """, unsafe_allow_html=True)
